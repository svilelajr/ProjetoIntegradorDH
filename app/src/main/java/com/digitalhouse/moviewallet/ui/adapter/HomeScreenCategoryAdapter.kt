...
class HomeScreenCategoryAdapter(private val listaDeCategorias: MutableList<Genre>) : RecyclerView.Adapter<HomeScreenCategoryAdapter.HomeScreenCategoryViewHolder>() {
    ...
    inner class HomeScreenCategoryViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        val nomeDasCategorias: TextView by lazy { view.findViewById<TextView>(R.id.txt_categorias) }
        val buttonVerMais: Button by lazy { view.findViewById<Button>(R.id.btn_ver_mais) }
        val recyclerViewFilmes: RecyclerView by lazy { view.findViewById<RecyclerView>(R.id.recyclerview_filmes) }
    }
}